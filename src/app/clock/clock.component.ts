...
export class ClockComponent implements OnInit {
  ...
  displayDate() {
    var d = new Date();
    var clock = document.getElementById("clock") as HTMLCanvasElement;
    clock.innerHTML = d.toLocaleTimeString();
    this.displayCanvas();
  }

  displayCanvas() {
    var canvasHTML = document.getElementById('myCanvas') as
      HTMLCanvasElement;
    var contextHTML = canvasHTML.getContext('2d') as CanvasRenderingContext2D;
    ...
    for (var th = 1; th <= 12; th++) {
      ...
      var xText = xCenterClock + (radiusNum - 25) * Math.cos(-30 * th * (Math.PI / 180) + Math.PI / 2);
      var yText = yCenterClock - (radiusNum - 25) * Math.sin(-30 * th * (Math.PI / 180) + Math.PI / 2);
      if (th <= 9) {
        contextHTML.strokeText(String(th), xText - 5, yText + 10);
      } else {
        contextHTML.strokeText(String(th), xText - 15, yText + 10);
      }
      contextHTML.stroke();
      contextHTML.closePath();
    }
    ...
    //Рисуем секунды
    contextHTML.beginPath();
    contextHTML.strokeStyle = "#8B0000";
    contextHTML.moveTo(xCenterClock, yCenterClock);
